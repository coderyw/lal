...
type HlsFileSystemNewer func() IFileSystemLayer
...
func FslFactory(t FslType, cacheDef HlsFileSystemNewer) IFileSystemLayer {
	switch t {
	case FslTypeDisk:
		return &FslDisk{}
	case FslTypeMemory:
	...
	case FlsTypeDefine:
		if cacheDef == nil {
			panic("使用自定义hls缓存配置时候，找不到缓存初始化函数")
		}
		return cacheDef()
	}
	return nil
...
func init() {
	DefaultDiskFileSystemLayer = FslFactory(FslTypeDisk, nil)
	DefaultMemoryFileSystemLayer = FslFactory(FslTypeMemory, nil)
}
